...
1. 如果表 T 中没有字段 k，而你执行了这个语句 select * from T where k=1, 那肯定是会报“不存在这个列”的错误： “Unknown column ‘k’ in ‘where clause’”。你觉得这个错误是在我们上面提到的哪个阶段报出来的呢？
   ...
   分析器先会做“词法分析”。MySQL从输入的"select"这个关键字识别出来，这是一个查询语句。它也要把字符串“T”识别成“表名 T”，把字符串“ID”识别成“列 ID”。

   分析器接着会做“语法分析”。根据词法分析的结果，语法分析器会根据语法规则，判断输入的这个SQL语句是否满足MySQL语法。

2. 判断一下你对这个表 T 有没有执行查询的权限是在哪个阶段？

    答：执行器。

    为什么不是分析器？有些时候，SQL语句要操作的表不只是SQL字面上那些。比如如果有个触发器，得在执行器阶段（过程中）才能确定。优化器阶段前是无能为力的
...
1. [为什么用B+树](https://www.sohu.com/a/280609547_818692)
    ...
    | 二叉搜索树 | 树的查找性能取决于树的高度，可能劣化成有序数组                                                            |
    | 平衡二叉树 | 平衡二叉树，时间复杂度也是$O(log(N))$；二叉树是搜索效率最高的，但是实际上大多数的数据库存储却并不使用二叉树。其原因是，索引不止存在内存中，还要写到磁盘上 |
    | B-tree | 文件系统和数据库的索引都是**存在硬盘上的**，并且如果数据量大的话，不一定能一次性加载到内存中                                   |
    | B+    | B+树可以更快地进行范围查询；多条的话，B树需要做局部的中序遍历，可能要跨层访问。而B+树由于所有数据都在叶子结点，不用跨层，同时由于有链表结构，只需要找到首尾，通过链表就能把所有数据取出来了                                                                    |
    
    100万节点的平衡二叉树，树高20。一次查询可能需要访问20个数据块。在机械硬盘时代，从磁盘随机读一个数据块需要10ms左右的寻址时间。也就是说，对于一个100万行的表，如果使用二叉树来存储，单独访问一个行可能需要20个10ms的时间
    
    InnoDB的一个整数字段索引为例，这个N差不多是1200。这棵树高是4的时候，就可以存1200的3次方个值，这已经17亿了。考虑到树根的数据块总是在内存中的，一个10亿行的表上一个整数字段的索引，查找一个值最多只需要访问3次磁盘。其实，树的第二层也有很大概率在内存中，那么访问磁盘的平均次数就更少了
...
- 基于主键索引和普通索引的查询有什么区别？
  ...
  如果语句是select * from T where k=5，即普通索引查询方式，则需要先搜索k索引树，得到ID的值为 500，再到ID索引树搜索一次。这个过程称为`回表`。
...
```
这个语句在搜索索引树的时候，只能用“张”，找到第一个满足条件的记录ID3。当然总比全表扫描要好。然后判断其他条件是否满足

优化之前: 5.6之前：只能从ID3开始一个个回表。到主键索引上找出数据行，再对比字段值

![](../images/无索引下推执行流程.png)

可以在索引遍历过程中，对索引中包含的字段先做判断，直接过滤掉不满足条件的记录(如第一个不满足age条件)，减少回表次数

![](../images/索引下推执行流程.png)

Questions:

样例：有这么一个表，表结构定义类似这样的：
```SQL
CREATE TABLE `geek` (
  `a` int(11) NOT NULL,
  `b` int(11) NOT NULL,
...
回答：
```Text
主键索引是 (a, b)，数据在存储时按照a和 b 的顺序进行组织。这意味着数据的物理存储顺序是先按a排序，再按b排序

索引 ca 的结构是 (c, a)，它按 c 和 a 排序。然而，单独的 c 索引已经可以快速定位到 c = N 的行，而 ca 索引只是在 c 的基础上增加了 a 的排序。由于主键已经按 a 和 b 排序，ca 索引的排序效果与单独的 c 索引基本一致，因此 ca 索引可以去掉。
索引 cb 的结构是 (c, b)，它按 c 和 b 排序。这个索引对于查询 select * from geek where c=N order by b limit 1; 是非常有用的，因为它可以直接利用索引中的顺序，避免额外的排序操作。因此，cb 索引需要保留
```

#### 索引相关问题

什么是主键索引，唯一索引，普通索引的区别？如何选择？

- 主键索引 (Primary Key Index)

  主键索引是一种特殊的唯一索引，用于唯一标识表中的每一行记录。一个表只能有一个主键索引，主键索引列的值不能重复且不能为 NULL。

- 唯一索引 (Unique Index)

  唯一索引确保某一个列或多个列的组合值是唯一的。一个表可以有多个唯一索引，唯一索引列的值可以包含 NULL。

- 普通索引 (Index)

  普通索引是最基本的索引类型，用于加速对表中数据的查询。普通索引允许重复值和 NULL 值。
...
假设一个市民系统，每个人都有一个唯一的身份证号，而且业务代码已经保证了不会写入两个重复的身份证号。如果市民系统需要按照身份证号查姓名，就会执行类似这样的SQL语句：
```SQL
select name from CUser where id_card = 'xxxxxxxyyyyyyzzzzz';
```

身份证号字段比较大，**不适合当**做主键，要么给 id_card 字段创建唯一索引，要么创建一个普通索引

查询过程：

![普通vs唯一.png](../images/普通vs唯一.png)

普通索引只比唯一索引多一次查询，由于**读一条记录的时候，并不是将这个记录本身从磁盘读出来，而是以页为单位，将其整体读入内存**,性能差异忽略不计
...
2. 某个业务的库内存命中率突然从99% 降低到了75%，整个系统处于阻塞状态，更新语句全部堵住

    是由于这个业务有大量插入数据的操作，而他在前一天把其中的某个普通索引改成了唯一索引
